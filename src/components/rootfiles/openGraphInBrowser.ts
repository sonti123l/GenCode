...
export function openGraphInBrowser(graph: any) {
  ...
  const html = generateHTML(safeJSON);
...
body{background:#111827;color:#fff;font-family:'Segoe UI',system-ui,sans-serif;height:100vh;overflow:hidden;display:flex;flex-direction:column;user-select:none}

/* ── header ── */
.header{display:flex;align-items:center;justify-content:space-between;padding:10px 20px;background:#1f2937;border-bottom:1px solid #374151;flex-shrink:0;gap:12px}
.header-left{display:flex;align-items:center;gap:10px}
.header-left h1{font-size:18px;font-weight:700;letter-spacing:-.3px}
.logo{width:28px;height:28px;background:linear-gradient(135deg,#3b82f6,#8b5cf6);border-radius:6px;display:flex;align-items:center;justify-content:center}
.logo svg{width:16px;height:16px}

/* ── layout buttons ── */
.layout-bar{display:flex;gap:4px;background:#374151;border-radius:8px;padding:3px}
.layout-btn{background:transparent;border:none;color:#9ca3af;padding:6px 14px;border-radius:6px;font-size:13px;cursor:pointer;transition:background .15s,color .15s;display:flex;align-items:center;gap:5px}
.layout-btn:hover{background:#4b5563;color:#fff}
.layout-btn.active{background:#2563eb;color:#fff}
.layout-btn:disabled{opacity:.35;cursor:not-allowed}

/* ── zoom controls ── */
.zoom-bar{display:flex;gap:4px}
.zoom-btn{background:#374151;border:none;color:#fff;width:32px;height:32px;border-radius:6px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s}
...
.stats-overlay{top:12px;left:12px;background:rgba(31,41,55,.9);backdrop-filter:blur(6px);border-radius:8px;padding:10px 14px;font-size:13px;line-height:1.7;color:#d1d5db}
.stats-overlay .label{color:#6b7280;font-size:11px;margin-top:4px}
.computing-badge{color:#facc15;font-size:11px;margin-top:2px;animation:pulse 1.4s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}
.hint{bottom:14px;left:50%;transform:translateX(-50%);background:rgba(37,99,235,.88);backdrop-filter:blur(6px);border-radius:8px;padding:8px 18px;font-size:13px;white-space:nowrap}

/* ── sidebar ── */
.sidebar{width:280px;background:#1f2937;border-left:1px solid #374151;overflow-y:auto;flex-shrink:0}
.sidebar-section{padding:16px;border-bottom:1px solid #374151}
.sidebar-section h3{font-size:15px;font-weight:600;margin-bottom:10px}
.sidebar-section h4{font-size:11px;text-transform:uppercase;color:#6b7280;letter-spacing:.5px;margin-bottom:8px;margin-top:12px}
.sidebar-section h4:first-of-type{margin-top:0}

/* node detail */
.node-detail-row{display:flex;align-items:center;gap:8px;font-size:13px;margin-bottom:6px}
.node-detail-row .lbl{color:#9ca3af}
.node-color-dot{width:14px;height:14px;border-radius:50%;flex-shrink:0}
.node-path{font-family:'Consolas',monospace;font-size:11px;background:#111827;padding:6px 8px;border-radius:5px;word-break:break-all;color:#60a5fa;margin-top:4px}

/* connection pills */
.conn-group-label{font-size:11px;color:#9ca3af;margin-bottom:6px;margin-top:10px}
.conn-pill{display:flex;align-items:center;justify-content:space-between;font-size:11px;background:rgba(17,24,39,.5);padding:5px 8px;border-radius:5px;margin-bottom:3px}
.conn-pill-left{display:flex;align-items:center;gap:6px}
.conn-pill-bar{width:14px;height:2px;border-radius:1px}

.clear-btn{margin-top:12px;width:100%;padding:7px;background:#374151;border:none;color:#fff;border-radius:6px;font-size:13px;cursor:pointer;transition:background .15s}
.clear-btn:hover{background:#4b5563}

/* legend */
.legend-row{display:flex;align-items:center;gap:8px;font-size:13px;margin-bottom:5px}
.legend-edge-line{width:24px;height:2px;border-radius:1px}
...
<div class="header">
  <div class="header-left">
    <div class="logo"><svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg></div>
  ...
  <div style="display:flex;align-items:center;gap:12px">
    ...
    <div class="zoom-bar">
      <button class="zoom-btn" id="zoomIn"><svg><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/><circle cx="11" cy="11" r="5"/><line x1="15" y1="15" x2="20" y2="20"/></svg></button>
      <button class="zoom-btn" id="zoomOut"><svg><line x1="8" y1="12" x2="16" y2="12"/><circle cx="11" cy="11" r="5"/><line x1="15" y1="15" x2="20" y2="20"/></svg></button>
      <button class="zoom-btn" id="resetView"><svg><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><path d="M21 15v6h-6"/><path d="M3 9V3h6"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg></button>
    </div>
  </div>
</div>
...
<div class="body">
  <div class="canvas-wrap" id="canvasWrap">
    ...
    <div class="overlay stats-overlay" id="statsOverlay">
      ...
      <div id="computingBadge" class="computing-badge" style="display:none">⏳ Computing layout…</div>
    </div>
    <div class="overlay hint" id="hint">💡 Click on any node to highlight its connections</div>
  </div>
    ...
    <div class="sidebar-section" id="legendSection">
      ...
      <div style="margin-top:18px;padding-top:12px;border-top:1px solid #374151">
        ...
        <ul style="font-size:11px;color:#6b7280;list-style:none;line-height:2">
          ...
          <li>• Switch layouts using the toolbar</li>
        </ul>
      </div>
    </div>
  </div>
...
const NODE_COLORS = {file:'#3B82F6',function:'#10B981',class:'#F59E0B',variable:'#8B5CF6',import:'#6B7280'};
const EDGE_COLORS = {CONTAINS:'#4B5563',CALLS:'#3B82F6',IMPORTS_FROM:'#10B981',USES:'#F59E0B',DEFINES:'#8B5CF6',EXTENDS:'#EF4444',EXPORTS:'#EC4899'};
...
let scale = 1;
let offset = {x:0, y:0};
let selectedNode = null;     // GraphNode | null
let layoutType = 'force';
let isComputing = false;

// pre-built adjacency
let neighborsMap = new Map();   // nodeId -> Set<nodeId>
let edgesByNode  = new Map();   // nodeId -> [edgeIndex, …]

// highlighted sets (rebuilt on selection)
let hlEdges = new Set();        // edge indices
let hlNodes = new Set();        // node ids

// ═══════════════════════════════════════════════════════════════════════════
// BUILD ADJACENCY (once)
// ═══════════════════════════════════════════════════════════════════════════
(function buildAdjacency(){
  GRAPH.nodes.forEach(n => { neighborsMap.set(n.id, new Set()); edgesByNode.set(n.id, []); });
...
// ═══════════════════════════════════════════════════════════════════════════
// WEB WORKER (inline blob) ── keeps main thread free during force layout
// ═══════════════════════════════════════════════════════════════════════════
const WORKER_SRC = \`
function forceLayout(nodes, edges){
  ...
  for(let s=0;s<STEPS;s++){
    ...
    // stream progress every 20 steps
    if(s%20===0||s===STEPS-1){
      const p={};
...
function draw(){
  ...
  // ── 1. Dimmed edges (when selection active) ──
  if(hasSel){
    ctx.save();
    ctx.globalAlpha=.1;
  ...
  // ── 2. Active edges (batched by color) ──
  ...
  buckets.forEach((indices,color)=>{
    ...
    // arrowheads + labels only for highlighted
    for(const i of indices){
      const e=edges[i];
      const fp=positions[e.from], tp=positions[e.to];
      if(!fp||!tp) continue;
      ...
      if(hasSel && hlEdges.has(i)){
        ...
        ctx.lineWidth=1.5;
        ctx.beginPath();
        // manual rounded rect (roundRect has no Safari support)
...
document.getElementById('zoomIn').addEventListener('click', ()=>{ scale=Math.min(6,scale*1.25); document.getElementById('zoomPct').textContent=(scale*100).toFixed(0); draw(); });
document.getElementById('zoomOut').addEventListener('click', ()=>{ scale=Math.max(.08,scale/1.25); document.getElementById('zoomPct').textContent=(scale*100).toFixed(0); draw(); });
document.getElementById('resetView').addEventListener('click', ()=>{ scale=1; offset={x:0,y:0}; document.getElementById('zoomPct').textContent='100'; draw(); });
